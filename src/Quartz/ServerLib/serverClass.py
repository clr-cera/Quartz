...
discovered_plugins = [
    ...
    for finder, name, ispkg in iter_namespace(ServerPlugins)
]
...
class Server:
    ...
    def changeUsername(self, message: messageLib.Msg) -> None:
        ...
        try:
            ...
            difference = (
                self.connections.index(message.sender) + 1 - len(self.usernames)
            )
    ...
    def CheckCActions(self, message: str, possibleCommand: str, msgObject: Msg) -> None:
        ...
        for plugin in self.plugins:
            try:
                if (
                    plugin.clientCommands(self, message, possibleCommand, msgObject)
                    == True
                ):
                    return True
            except:
